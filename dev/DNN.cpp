...
public:
	...
	void feed_and_propogate(Matrix& input_array, Matrix& target_array, int epochs, int batch_size) {
		...
		std::vector<int> batches_shape = {ceil(input_all.shape[0]/batch_size), batch_size, 2, 1}; Matrix batches(batches_shape);
		for(int i = 0; i < input_all.shape[0]; ++i){
			if(i % batch_size == 0)
		...
		for(int e = 0; e < epochs; ++e){
			...
			for(int b = 0; b < batches.num_vals; ++b){
				...
				for(int p = 0; p < batch->num_vals; ++p){
					...
					Matrix* hidden1_errors = Matrix::DotProduct(*weights1_tr, *last_errors); //last_errors probably isn't defined correctly anymore. fix that.
